...
async def get_payment_inquiry_by_line_user_id(line_user_id: str, db: AsyncSession = Depends(get_db_session)) -> PaymentInquiryResponse:
    service = PaymentService(ContractRepository(db), LineMappingRepository(db), PaymentRepository(db))
...
async def get_payment_history_by_line_user_id(line_user_id: str, db: AsyncSession = Depends(get_db_session)) -> list[PaymentHistoryItemResponse]:
    service = PaymentService(ContractRepository(db), LineMappingRepository(db), PaymentRepository(db))
